...
2. **Never guess a marketing block's import statement.** Marketing block files use a *mix* of
...
and both are full sections rather than primitives. The difference is what they are *for*:
...
- **Count it against the motion budget.** An animated background *is* the section's one
...
|---|---|
| `claude` *(default)* | `.claude/skills/<slug>/SKILL.md` |
...
- **Install component** *(Starter+)*: `npx shadcn@latest add @reactbits-starter/<slug>-tw` (or `-css`)
- **Install marketing block** *(Pro+)*: `npx shadcn@latest add @reactbits-pro/<slug>`
- **Install App UI block** *(Pro+)*: `npx shadcn@latest add @reactbits-pro/<slug>`
- **Install Agent Kit item** *(Pro+)*: `npx shadcn@latest add @reactbits-pro/{skill,prompt,recipe}-<slug>`
- **Install this skill** *(Starter+)*: `npx shadcn@latest add @reactbits-starter/skill`
