...
import type { UserVideoTableRow } from "@/lib/clientVideoUnlock";
import { UnlockDatetimeField } from "@/components/admin/UnlockDatetimeField";

export type { UserVideoTableRow as AdminUserVideoRow };

type UserVideosTableProps = {
  clientId: string;
  rows: UserVideoTableRow[];
  updateUnlock: (
    clientId: string,
    videoId: string,
    unlockAtLocal: string
  ) => Promise<{ ok: true }>;
  reseedUnlocks: (clientId: string) => Promise<{ ok: true; count?: number | null }>;
};
...
function VideoRowEditor({
  ...
  clientId: string;
  row: UserVideoTableRow;
  updateUnlock: UserVideosTableProps["updateUnlock"];
}) {
      ...
      <td className="px-3 py-3">
          ...
          <button
            ...
            onClick={() => {
              ...
              startTransition(async () => {
                try {
                  await updateUnlock(clientId, row.videoId, localValue);
        ...
        {row.unlockSource && (
          <p className="mt-1 text-xs text-white/40">
            Quelle:{" "}
            {row.unlockSource === "default"
              ? "Standard"
              : row.unlockSource === "manual"
                ? "Manuell"
                : row.unlockSource}
          </p>
        )}
        {!row.unlockAt && row.globalPosition < 4 && (
          <p className="mt-1 text-xs text-white/40">
            Optional: Zeitplan für sequentielle Videos
          </p>
        )}
        {message && (
          <p
...
export function UserVideosTable({
  clientId,
  rows,
  updateUnlock,
  reseedUnlocks,
}: UserVideosTableProps) {
  const router = useRouter();
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button
          ...
          onClick={() => {
            ...
            startReseed(async () => {
              try {
                const result = await reseedUnlocks(clientId);
                setReseedMsg(
      ...
      {reseedMsg && (
        <p
          className={`text-sm ${reseedMsg.includes("Fehler") ? "text-red-300" : "text-[#63eca9]"}`}
        >
          {reseedMsg}
        </p>
      )}
