...
const getAnchorAndDir = (origin: Origin, w: number, h: number) => {
  ...
  switch (origin) {
    case "top-left":
      return { anchor: [0, -outside * h], dir: [0, 1] } as const;
    case "top-right":
      return { anchor: [w, -outside * h], dir: [0, 1] } as const;
    case "left":
      return { anchor: [-outside * w, 0.5 * h], dir: [1, 0] } as const;
    case "right":
      return { anchor: [(1 + outside) * w, 0.5 * h], dir: [-1, 0] } as const;
    case "bottom-left":
      return { anchor: [0, (1 + outside) * h], dir: [0, -1] } as const;
    case "bottom-center":
      return { anchor: [0.5 * w, (1 + outside) * h], dir: [0, -1] } as const;
    case "bottom-right":
      return { anchor: [w, (1 + outside) * h], dir: [0, -1] } as const;
    default:
      return { anchor: [0.5 * w, -outside * h], dir: [0, 1] } as const;
  }
};
...
export function LightRays({
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      ...
      const renderer = new Renderer({ dpr: Math.min(window.devicePixelRatio, 2), alpha: true }) as unknown as MinimalRenderer;
      ...
      while (containerRef.current.firstChild) containerRef.current.removeChild(containerRef.current.firstChild as Node);
      ...
      const colorVec = hexToRgb(raysColor) as [number, number, number];
      const uniforms: Uniforms = {
        iTime: { value: 0 },
        iResolution: { value: [1, 1] },
        rayPos: { value: [0, 0] },
        rayDir: { value: [0, 1] },
        raysColor: { value: colorVec },
      ...
      const geometry = new Triangle(gl as OGLRenderingContext);
      const program = new Program(gl as OGLRenderingContext, { vertex: vert, fragment: frag, uniforms: uniforms as unknown as Record<string, unknown> });
      const mesh = new Mesh(gl as OGLRenderingContext, { geometry, program });
      ...
      const updatePlacement = () => {
        ...
        uniforms.rayPos.value = [anchor[0], anchor[1]];
        uniforms.rayDir.value = [dir[0], dir[1]];
      };
  ...
  useEffect(() => {
    ...
    u.raysColor.value = hexToRgb(raysColor) as [number, number, number];
    ...
    const { clientWidth: wCSS, clientHeight: hCSS } = containerRef.current;
    const dpr = renderer.dpr;
    const { anchor, dir } = getAnchorAndDir(raysOrigin as Origin, wCSS * dpr, hCSS * dpr);
    u.rayPos.value = [anchor[0], anchor[1]];
    u.rayDir.value = [dir[0], dir[1]];
  ...
  return <div ref={containerRef} className={`pointer-events-none relative z-10 ${className}`} />;
}
